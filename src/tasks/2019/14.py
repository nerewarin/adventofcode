        ...
        for line in lines:
            *consumes, produce = self._reaction_rexp.findall(line)
            expressions[produce[1]] = {
                "count": int(produce[0]),
                "ingredients": {consume[1]: int(consume[0]) for consume in consumes},
            }
...
def test(test_num, mode=0):
    ...
    if test_num == 1:
        ...
        assert res == 31, f"test{test_num} failed!: {res}"
    if test_num == 2:
        res = SpaceStoichiometry(inp2).get_ore_for_fuel()
        assert res == 165, f"test{test_num} failed!: {res}"
    if test_num == 3:
        _inp = inp3
        if not mode:
            res = SpaceStoichiometry(_inp).get_ore_for_fuel()
            assert res == 13312, f"test{test_num} failed!: {res}"
        else:
            res = get_fuel_for_ore(ore_mode_2, _inp)
            assert res == 82892753, f"test{test_num} failed!: {res}"

    if test_num == 4:
        _inp = inp4
        if not mode:
            res = SpaceStoichiometry(inp4).get_ore_for_fuel()
            assert res == 180697, f"test{test_num} failed!: {res}"
        else:
            res = get_fuel_for_ore(ore_mode_2, _inp)
            assert res == 5586022, f"test{test_num} failed!: {res}"

    if test_num == 5:
        _inp = inp5
        if not mode:
            res = SpaceStoichiometry(_inp).get_ore_for_fuel()
            assert res == 2210736, f"test{test_num} failed!: {res}"
        else:
            res = get_fuel_for_ore(ore_mode_2, _inp)
            assert res == 460664, f"test{test_num} failed!: {res}"
